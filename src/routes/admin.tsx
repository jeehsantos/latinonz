...
function SiteModePanel() {
  ...
  useEffect(() => setMode(getStoredSiteMode()), []);
  const isLive = mode === "live";

  const toggle = () => {
    const next: SiteMode = isLive ? "waitlist" : "live";
    setStoredSiteMode(next);
    setMode(next);
  };
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-lg font-extrabold text-gray-900">Modo do site</h2>
          <p className="text-sm text-gray-500">
            {isLive
              ? "A plataforma está ativa para todos os visitantes."
              : "Apenas a lista de espera está visível para o público."}
          </p>
        </div>
        <div className="flex items-center gap-3">
          <a
            href="/?preview=platform"
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 text-sm font-bold text-[#1A5336] border border-[#1A5336]/20 bg-[#EBF4ED] px-3 py-2 rounded-lg hover:bg-[#1A5336]/10"
          >
            <ExternalLink size={14} /> Pré-visualizar plataforma
          </a>
          <button
            onClick={toggle}
            className={`inline-flex items-center gap-2 font-bold px-4 py-2 rounded-lg text-sm ${
              isLive ? "bg-[#1A5336] text-white" : "bg-gray-100 text-gray-700"
            }`}
          >
