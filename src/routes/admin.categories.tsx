...
import {
  ICON_KEYS, ICON_MAP, COLOR_KEYS, COLOR_MAP,
  type IconKey, type ColorKey,
...
function AdminCategoriesPage() {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => { setOpen(false); setForm(EMPTY_FORM); setError(null); invalidate(); },
  ...
  const openCreate = () => { setForm(EMPTY_FORM); setError(null); setOpen(true); };
  const openEdit = (r: NonNullable<typeof data>["categories"][number]) => {
    setForm({
      id: r.id,
      namePt: r.namePt, nameEs: r.nameEs, nameEn: r.nameEn,
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Cadastre categorias com ícone e traduções para PT / ES / EN.</p>
        </div>
        <button
          onClick={openCreate}
        ...
        <table className="w-full text-left">
          ...
          <tbody className="text-sm divide-y divide-gray-100">
            {isLoading ? (
              ...
              <tr><td colSpan={5} className="p-8 text-center text-gray-400">Nenhuma categoria cadastrada.</td></tr>
            ) : rows.map((r) => {
              const Icon = ICON_MAP[(r.iconKey as IconKey) in ICON_MAP ? (r.iconKey as IconKey) : "briefcase"];
              const color = COLOR_MAP[(r.colorKey as ColorKey) in COLOR_MAP ? (r.colorKey as ColorKey) : "slate"];
              return (
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-4">
                    <div className={`w-10 h-10 rounded-2xl ${color.bg} ${color.text} flex items-center justify-center`}>
                      <Icon size={18} />
                    </div>
                  </td>
                  <td className="p-4 text-xs text-gray-600 space-y-0.5">
                    <div><span className="font-bold text-gray-700">ES:</span> {r.nameEs || <span className="text-gray-300">—</span>}</div>
                    <div><span className="font-bold text-gray-700">EN:</span> {r.nameEn || <span className="text-gray-300">—</span>}</div>
                  </td>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                ...
                {(["pt", "es", "en"] as const).map((lng) => {
                  ...
                  const nameField = (`name${lng.charAt(0).toUpperCase() + lng.slice(1)}`) as "namePt" | "nameEs" | "nameEn";
                  const blurbField = (`blurb${lng.charAt(0).toUpperCase() + lng.slice(1)}`) as "blurbPt" | "blurbEs" | "blurbEn";
                  return (
                    <div key={lng} className="grid md:grid-cols-2 gap-3">
                      <div>
                        <label className="text-xs font-bold text-gray-600">{labels} — Nome {lng === "pt" && <span className="text-red-500">*</span>}</label>
                        <input
                      ...
                      <div>
                        <label className="text-xs font-bold text-gray-600">{labels} — Descrição curta</label>
                        <input
              ...
              {/* Icon picker */}
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Ícone</p>
                ...
                <div className="flex flex-wrap gap-2">
                  {COLOR_KEYS.map((k) => {
                    ...
                    return (
                      ...
                      >
                        <span className={`w-4 h-4 rounded-md ${c.bg} ${c.text} flex items-center justify-center`}>
